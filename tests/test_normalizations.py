...
class TestNormalizationsMethods(unittest.TestCase):
    ...
    def test_normalize_zip(self):
        ...
        self.assertTrue(self.entries_true[0].zip_code == None or (valid_zip(self.x.entries[0],"zip_code")).status =="passed")

    def test_normalize_state(self):
        normalize_state(self.entries_true[0],"state")
        self.assertEqual(valid_state(self.entries_true[0],"state").status,"passed")
